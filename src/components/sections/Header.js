        ...
        <header className="pb-10 pt-8 md:pt-20">
            ...
            <nav className="mt-10 flex flex-wrap items-baseline gap-x-7 gap-y-3">
                {siteContent.navigation.map(({ key, label, to, href, external, count }) => {
                    const classes = 'group no-underline inline-flex items-baseline gap-1.5 text-xs font-bold uppercase tracking-widest text-neutral-500 hover:text-neutral-900 transition-colors';
                    const content = (
                        <>
